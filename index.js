...
let song = ["And It Stoned Me Van", "Atlantic City", "Behind Blue Eyes", "Blackbird","Brown Eyed Girl", "Build Me Up Buttercup","California Stars",
    "Don't Let It Bring You Down","Down By the Water","Elderly Woman","Everybody Hurts","Handle With Care","Heart of Gold", "Helplessly Hoping",
    "Hotel California", "If You Could Read My Mind", "Into the Mystic ", "I Will Survive Gloria"];

/* Game */
const category = [song, movies]
const youWon = "You Won!";
const youLost = "You Lost!";

function Game()
{
    let pickCategory = category[Math.floor(Math.random() * category.length)];
    let pick = pickCategory[Math.floor(Math.random() * pickCategory.length)].toUpperCase();
	let guessedLetters = [];
	let maskedWord = "";
    ...
    let categoryName = pickCategory === song ? "Songs" : "Movies"; // Choose category label
    let choose = document.getElementById("p-category");
    choose.innerHTML = `Category: ${categoryName}`;
	...
	{
		let space = " ";
		let nextCharacter = pick.charAt(i) === space ? space : "_";
	...
	let guess = function( letter ) 
	{
		...
		{	
			...
			if( pick.includes( letter ) )
			{
				let matchingIndexes = [];
				for ( let i = 0; i < pick.length; i++ ) 
				{
					if( pick.charAt(i) === letter )
					{
